...
#           2. randomize# raised from 24000 — see conversation notes on pacing_positions jitter around the base pair.
...
#     def reset(self):
#         self.client.simPause(False)  # sim is left paused from the previous episode's last step()
#         self.client.reset()
#         self.client.enableApiControl(True)
#         self.client.armDisarm(True)
#         self.client.takeoffAsync().join()
...
#         obs = self._get_observation()
#         self.client.simPause(True)  # freeze here; step() unpauses only for its own movement command
#         return obs
...
#         # Unpause only for the duration of the actual movement command, then
#         # immediately pause again. While paused, the physics engine is fully
#         # frozen — no gravity, no drift — so whatever time observation-building
#         # (simGetImages for depth/segmentation, collision check, etc.) takes
#         # afterward has zero effect on the drone's real position. This was
#         # confirmed via step_only_test.py: without pausing, sustained forward
#         # flight showed a small but consistent downward drift (~0.045m/call)
#         # purely from real time elapsing during simGetImages() with no active
#         # command — simPause(True) eliminates that gap entirely rather than
...
class AirSimEnv:
    """
    Gym-style environment wrapping AirSim, config-driven per environment
    (Blocks, City, etc. — see configs/<env_name>.yaml).
    ...
    Observation vector (20 dims):
        ...
        depth obstacle features (5): min depth per horizontal zone, normalized
        segmentation obstacle features (5): obstacle pixel density per zone,
            using explicit semantic classes defined per-environment in that
            environment's YAML config (segmentation_classes: class id ->
            list of regex patterns matched against real Unreal mesh names
            via simSetSegmentationObjectID). Genuinely semantic, not a
            raw-color proxy, since classes are deliberately assigned based
            on each map's actual mesh names — see list_scene_objects.py for
            how to build this mapping for a new environment.
    ...
    def __init__(self, env_name="city", configs_dir=None,
                 ...
                 **overrides):
        """
        env_name: which configs/<env_name>.yaml to load (e.g. "blocks", "city").
        configs_dir: directory containing the yaml files. Defaults to a
            "configs" folder next to this file.
        overrides: any config key can be overridden directly at construction
            time, e.g. AirSimEnv(env_name="blocks", accept_radius=1.5) —
            useful for quick experiments without editing the yaml.
        """
        self.env_name = env_name
        self.config = self._load_config(env_name, configs_dir, overrides)
        ...
        # base/reference positions — used directly if no randomization is
        # configured, or as the center of the jitter sphere if it is
        self.base_start_position = np.array(self.config["start_position"], dtype=np.float32)
        self.base_goal_position = np.array(self.config["goal_position"], dtype=np.float32)
        ...
        self.scenario_pairs = scenario_pairs
        self.randomize_positions = randomize_positions
        self.start_jitter_radius = self.config["start_jitter_radius"]
        self.goal_jitter_radius = self.config["goal_jitter_radius"]
        self.max_reset_attempts = max_reset_attempts

        self.max_episode_steps = max_episode_steps
        self.accept_radius = self.config["accept_radius"]
        self.max_depth_meters = self.config["max_depth_meters"]

        wb = self.config["workspace_bounds"]
        self.workspace_bounds = {
            "x": tuple(wb["x"]), "y": tuple(wb["y"]), "z": tuple(wb["z"]),
        }
    ...
    def _load_config(self, env_name, configs_dir, overrides):
        if configs_dir is None:
            configs_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "../configs")
        config_path = os.path.join(configs_dir, f"{env_name}.yaml")

        if not os.path.exists(config_path):
            raise FileNotFoundError(
                f"No config found for env_name='{env_name}' at {config_path}. "
                f"Expected a YAML file there (see configs/blocks.yaml for the format)."
            )

        with open(config_path, "r") as f:
            config = yaml.safe_load(f)

        if config.get("segmentation_classes") == {}:
            print(f"Warning: configs/{env_name}.yaml has an empty segmentation_classes — "
                  f"segmentation features will be all-zero until this is filled in "
                  f"(see list_scene_objects.py).")
    ...
    def _configure_segmentation_classes(self):
        """
        Assigns semantic segmentation IDs based on this environment's config
        (segmentation_classes: class id -> list of regex patterns, matched
        against real Unreal mesh names). Everything not matched by any
        pattern is left at its default ID — non-physical scene elements
        (cameras, lights, sky, UI actors), not real obstacles.

        Called once at env init so every episode uses the same mapping.
        """
        seg_classes = self.config.get("segmentation_classes", {})
        for class_id, patterns in seg_classes.items():
            for pattern in patterns:
                self.client.simSetSegmentationObjectID(pattern, int(class_id), True)
    ...
    def step(self, action_idx):
        ...
        # frozen — no gravity, no drift — so whatever time observation-building
        # (simGetImages for depth/segmentation, collision check, etc.) takes
        # afterward has zero effect on the drone's real position.
        self.client.simPause(False)
        self.client.moveByVelocityAsync(
            vx, vy, vz, self.dt,
            drivetrain=airsim.DrivetrainType.MaxDegreeOfFreedom,
